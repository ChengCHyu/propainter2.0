...
# 尝试多个可能的dependencies位置
possible_roots = [
    os.path.dirname(current_dir),  # 项目根目录 (如果member_b在根目录)
    os.path.join(os.path.dirname(current_dir), 'video_object_removal', 'src'),  # video_object_removal/src
]
dependencies_path = None
src_root = None
for root in possible_roots:
    test_path = os.path.join(root, 'dependencies')
    if os.path.exists(test_path):
        dependencies_path = test_path
        src_root = root
        break

if dependencies_path is None:
    # 回退到原始路径
    src_root = os.path.join(os.path.dirname(current_dir), 'video_object_removal', 'src')
    dependencies_path = os.path.join(src_root, 'dependencies')

# 再次清理sys.path（防止依赖路径操作引入cv2目录）
_cleaned_paths2 = []
for p in sys.path:
    normalized = p.replace('\\', '/').rstrip('/')
    if normalized.endswith('/cv2'):
        continue
    _cleaned_paths2.append(p)
sys.path = _cleaned_paths2
