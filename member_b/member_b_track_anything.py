...
利用现有的Track-Anything实现，添加边界框输入支持
"""
...
# 添加项目路径
# member_b目录的路径
member_b_dir = os.path.dirname(os.path.abspath(__file__))
# 项目根目录（member_b的上一级目录）
project_root = os.path.dirname(member_b_dir)
sys.path.append(project_root)
sys.path.append(os.path.join(project_root, 'web-demos', 'hugging_face'))
...
class MemberBTrackAnything:
    ...
    def __init__(self,
        ...
        if sam_checkpoint is None:
            # 默认使用项目根目录的weights
            default_sam_path = os.path.join(project_root, 'weights', 'sam_vit_h_4b8939.pth')
            sam_checkpoint = os.environ.get('SAM_CHECKPOINT', default_sam_path)
        
        # 如果是相对路径，尝试在项目根目录查找
        if not os.path.isabs(sam_checkpoint) and not os.path.exists(sam_checkpoint):
            project_weights_path = os.path.join(project_root, sam_checkpoint)
            if os.path.exists(project_weights_path):
                sam_checkpoint = project_weights_path
                print(f"[成员B] 使用项目根目录的SAM模型: {sam_checkpoint}")
        ...
        if not os.path.exists(sam_checkpoint):
            ...
            if sam_model_type in sam_checkpoint_url_dict:
                # 确保weights目录存在（使用项目根目录的weights）
                if os.path.isabs(sam_checkpoint):
                    weights_dir = os.path.dirname(sam_checkpoint)
                else:
        ...
        if tracker_checkpoint is None:
            # 默认使用项目根目录的weights
            default_tracker_path = os.path.join(project_root, 'weights', 'cutie-base-mega.pth')
            tracker_checkpoint = os.environ.get('TRACKER_CHECKPOINT', default_tracker_path)
        
        # 如果是相对路径，尝试在项目根目录查找
        if not os.path.isabs(tracker_checkpoint) and not os.path.exists(tracker_checkpoint):
            project_weights_path = os.path.join(project_root, tracker_checkpoint)
            if os.path.exists(project_weights_path):
                tracker_checkpoint = project_weights_path
                print(f"[成员B] 使用项目根目录的CUTIE模型: {tracker_checkpoint}")
        ...
        if not os.path.exists(tracker_checkpoint):
            ...
            # 确保weights目录存在（使用项目根目录的weights）
            if os.path.isabs(tracker_checkpoint):
                weights_dir = os.path.dirname(tracker_checkpoint)
            else:
                weights_dir = os.path.join(project_root, 'weights')
            os.makedirs(weights_dir, exist_ok=True)
    ...
    def segment_first_frame(self,
        ...
        for obj_id, bbox in enumerate(bboxes, start=1):
            ...
            # 调试：打印分割结果
            print(f"[成员B] 物体{obj_id}分割完成: 边界框={bbox}, 掩码非零像素={np.sum(mask_binary > 0)}")
    ...
    def track_video_bidirectional(self,
        ...
        """
        双向追踪：从指定帧开始向前后两个方向追踪
        
        Args:
            video_frames: 视频帧列表，每个帧为 (H, W, 3) RGB格式
            start_mask: 起始帧的掩码 (H, W)，不同物体用不同值标记
            start_frame: 起始帧索引
        
        Returns:
            tracked_masks: 追踪后的掩码列表（包含所有帧）
        """
        total_frames = len(video_frames)
        tracked_masks = [None] * total_frames
        
        print(f"[成员B] 开始双向追踪，从第 {start_frame} 帧开始，共 {total_frames} 帧")
        
        # 初始化起始帧的掩码
        tracked_masks[start_frame] = start_mask.copy()
        
        # 向后追踪（start_frame+1 到 最后）
        if start_frame < total_frames - 1:
            print(f"[成员B] 向后追踪：第 {start_frame+1} 帧到第 {total_frames-1} 帧")
            self.tracker.clear_memory()
        ...
        if start_frame > 0:
            print(f"[成员B] 向前追踪：第 {start_frame-1} 帧到第 0 帧")
            self.tracker.clear_memory()
            
            # 创建反向帧序列并追踪（从start_frame反向到0）
            reverse_frames = video_frames[start_frame::-1]  # 从start_frame反向到0
            reverse_masks = []
            
            for i, frame in enumerate(reverse_frames):
                if i == 0:
                    # 第一帧（原起始帧）使用原始掩码
                    mask, _, _ = self.tracker.track(frame, first_frame_annotation=start_mask)
                else:
                    mask, _, _ = self.tracker.track(frame)
                reverse_masks.append(mask)
            
            # 将反向追踪的结果填入tracked_masks（注意顺序要反转回来）
            for i, mask in enumerate(reverse_masks):
                frame_idx = start_frame - i
                tracked_masks[frame_idx] = mask
        
        print(f"[成员B] 双向追踪完成，生成 {len([m for m in tracked_masks if m is not None])} 个掩码")
    ...
    def process_video(self,
        ...
        """
        处理整个视频：从指定帧分割 + 双向追踪所有帧
        
        Args:
            video_path: 视频路径（可以是视频文件或帧文件夹）
            bboxes: 边界框，可以是：
                    - List[List[float]]: 起始帧的边界框列表
        ...
        if start_frame < 0 or start_frame >= len(frames):
            raise ValueError(f"起始帧索引 {start_frame} 超出范围 [0, {len(frames)-1}]")
        
        # 处理边界框输入
        if isinstance(bboxes, dict):
            # 如果提供了多帧的边界框，使用起始帧的
            start_frame_bboxes = bboxes.get(start_frame, [])
            if len(start_frame_bboxes) == 0:
                # 如果没有找到起始帧的边界框，尝试使用第一个可用的
                start_frame_bboxes = bboxes.get(list(bboxes.keys())[0], [])
        else:
        ...
        # 分割起始帧（不是第一帧）
        if start_frame == 0:
            print(f"[成员B] 分割第 {start_frame} 帧（第一帧），检测到 {len(start_frame_bboxes)} 个物体")
        else:
            print(f"[成员B] 分割第 {start_frame} 帧，检测到 {len(start_frame_bboxes)} 个物体")
        
        start_frame_mask, mask_info = self.segment_first_frame(frames[start_frame], start_frame_bboxes)
        
        # 追踪所有帧（使用双向追踪）
        if start_frame == 0:
            # 如果从第一帧开始，使用原来的单向追踪（更简单高效）
            tracked_masks = self.track_video(frames, start_frame_mask)
        else:
            # 如果从中间帧开始，使用双向追踪
            tracked_masks = self.track_video_bidirectional(frames, start_frame_mask, start_frame)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='成员B：基于Track-Anything的分割与追踪模块')
    parser.add_argument('--video', type=str, required=True, help='输入视频路径')
    parser.add_argument('--bbox', type=str, required=True, help='边界框JSON文件路径或直接输入格式: "[[x1,y1,x2,y2]]"')
