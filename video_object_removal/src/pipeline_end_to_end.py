...
# 导入ProPainter（成员C）- 需要父目录的ProPainter代码
# 如果不需要端到端流程，可以注释掉这部分
PROPANTER_AVAILABLE = False
try:
    # 尝试导入ProPainter（需要父目录）
    parent_root = os.path.dirname(os.path.dirname(src_root))
    if os.path.exists(os.path.join(parent_root, 'inference_propainter.py')):
    ...
    print(f"提示: 端到端流程需要父目录的ProPainter代码")
    PROPANTER_AVAILABLE = False
